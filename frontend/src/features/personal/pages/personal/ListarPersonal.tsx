...
export default function ListarPersonal() {
  ...
  const [filtro, setFiltro] = useState<"todos" | "operar" | "administrar">(
    "todos",
  );
  ...
  const totalPaginas = Math.max(
    1,
    Math.ceil(personalFiltrado.length / PAGE_SIZE),
  );

  // Si el filtro deja la página actual fuera de rango, la corregimos
  useEffect(() => {
  ...
  const personalPaginado = useMemo(() => {
    ...
    return personalFiltrado.slice(inicio, inicio + PAGE_SIZE);
  }, [personalFiltrado, pagina]);

  const cambiarFiltro = (nuevoFiltro: typeof filtro) => {
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      >
        ...
        <Link to="/personal/nuevo" style={estiloBotonAdd}>
          + Agregar
        </Link>
      </div>
      ...
      <div style={{ marginBottom: "20px", display: "flex", gap: "10px" }}>
        <button
          ...
          style={estiloFiltro(filtro === "todos")}
        >
          Todos
        </button>
        <button
          onClick={() => cambiarFiltro("operar")}
          style={estiloFiltro(filtro === "operar")}
        >
          Operadores
        </button>
        <button
          onClick={() => cambiarFiltro("administrar")}
          style={estiloFiltro(filtro === "administrar")}
        >
          Administradores
        </button>
      </div>
          ...
          {personalPaginado.map((p) => (
            <tr key={p.id} style={{ borderBottom: "1px solid #eee" }}>
              ...
              <td style={{ padding: "12px", textAlign: "center" }}>
                <button
                  onClick={() => navigate(`/personal/detalle/${p.id}`)}
                  style={{
                    background: "none",
                    border: "none",
                    cursor: "pointer",
              ...
              >
                <button
                  ...
                  style={estiloBotonAccion("editar")}
                >
                  Modificar
                </button>
                <button
                  onClick={() => setIdABorrar(p.id!)}
                  style={estiloBotonAccion("eliminar")}
                >
                  Eliminar
                </button>
              </td>
        ...
        >
          {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((num) => (
            <button
              ...
              style={estiloBotonPagina(num === pagina, false)}
            >
              {num}
            </button>
          ))}
...
const estiloBotonAccion = (tipo: "editar" | "eliminar") => ({
  backgroundColor: tipo === "editar" ? "#f0ad4e" : "#d9534f",
  color: "white",
  border: "none",
  padding: "6px 12px",
  borderRadius: "4px",
...
const estiloBotonPagina = (activo: boolean, deshabilitado: boolean) => ({
  padding: "6px 12px",
  borderRadius: "6px",
  border: "1px solid #468189",
  backgroundColor: activo ? "#468189" : "#fff",
  color: deshabilitado ? "#ccc" : activo ? "#fff" : "#468189",
  borderColor: deshabilitado ? "#ccc" : "#468189",
